...
export class PhantasmaLink {
  constructor(dappID) {
    ...
    this.onNoWalletDetected = null;
  }

  login(callback, cancelCallback, noWalletCallback) {
    this.onLogin = callback;
    this.onCancel = cancelCallback;
    this.onNoWalletDetected = noWalletCallback;
    this.createSocket();
  }
  ...
  //   this.sendLinkRequest("signData/" + data + "/" + signature + "/phantasma", function (result) {
  //     callback(result);
  //     if (result.success && !result.hash.error) {
  //       alertbox.show("Transaction successful, hash: " + result.hash);
  //       console.log("Transaction successful, hash: " + result.hash);
  ...
  signData(data, callback, onErrorCallback) {
    ...
    if (data.data) {
        data = this.getSignature(data.data)
    }

    console.log('[signData] data', data);
  ...
  createSocket() {
    ...
    try {
      ...
      alert(
        "Couldn't connect to any Phantasma wallet. To participate in the sale, you need a wallet. Type 'download phantasma wallet' in your favorite search engine and install one. There is the browser extension wallet Ecto, or the standalone Poltergeist desktop/mobile application."
      );
    }
    ...
    this.socket.onclose = function (event) {
      ...
      if (!event.wasClean) {
        alertbox.show("Error: connection terminated... Are you sure you have a compatible Phantasma wallet?");
        console.log("Error: connection terminated... Are you sure you have a compatible Phantasma wallet?");
        if (that.onNoWalletDetected) {
          that.onNoWalletDetected();
        } else {
          alert(
            "Couldn't connect to any Phantasma wallet. To participate in the sale, you need a wallet. Type 'download phantasma wallet' in your favorite search engine and install one. There is the browser extension wallet Ecto, or the standalone Poltergeist desktop/mobile application."
          );
...
export function disconnectLink(triggered) {
  if (triggered) {
